# Fort Bad

![Fort Bad](../../assets/areas/5FortBad.png){ width=400 loading=lazy }

A fort dedicated to Badspot. Rocket Orcs spawn outside.

Nearby points of interest:

- [Tavern](tavern.md)
- [Level 2](level-2.md)

## Interior

The fort has a **main floor** and a **second floor** reachable inside.

## Hidden room

There is a hidden room tucked into one of the main floor walls. Look along
the walls for a small **gap above one of them** - that gap is the tell for
where the entrance is.

To get in, stand close to the wall and while pressing **W** to run forwards also press **jump**. You will pass
through the gap and land inside the hidden room. This may take a few attempts to get in. The room itself is small,
just **five floors** stacked on top of each other with nothing else in it.

## Screenshots

<div class="grid cards" markdown>

- ![Fort Bad viewed from above](../../assets/captures/fort-bad/view-from-above.png){ loading=lazy data-gallery="fort-bad" }

    **View from above** - the fort seen from overhead.

- ![Fort Bad viewed from above at another angle](../../assets/captures/fort-bad/view-from-above-2.png){ loading=lazy data-gallery="fort-bad" }

    **Another view from above** - a second overhead angle of the fort.

- ![Fort Bad viewed from above looking at the front](../../assets/captures/fort-bad/view-from-above-3-front.png){ loading=lazy data-gallery="fort-bad" }

    **View from above (front)** - overhead angle looking at the front of the
    fort.

- ![Fort Bad main floor interior](../../assets/captures/fort-bad/main-floor.png){ loading=lazy data-gallery="fort-bad" }

    **Main floor** - inside view of the fort's main floor.

- ![Fort Bad second floor interior](../../assets/captures/fort-bad/second-floor.png){ loading=lazy data-gallery="fort-bad" }

    **Second floor** - inside view of the fort's second floor.

- ![Fort Bad hidden room entrance](../../assets/captures/fort-bad/hidden-room-entrance.png){ loading=lazy data-gallery="fort-bad" }

    **Hidden room entrance** - the gap above the wall that marks where to
    jump.

- ![Inside the Fort Bad hidden room](../../assets/captures/fort-bad/hidden-room-inside.png){ loading=lazy data-gallery="fort-bad" }

    **Inside the hidden room** - the five stacked floors inside.

</div>
